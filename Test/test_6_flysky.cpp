...
extern "C" {
    ...
    extern int packetpersecond; // rx_flysky.cpp
    extern float vbattfilt; // battery.c
    extern const char *tprintf(const char* fmt, ...);
}

//------------------------------------------------------------------------------
// Expected by afhds2a_support.c, writeEEPROM()
extern "C" void flash_save(void)
{
...
static void update_osd()
{
    ...
    osd_print(row++, 1, tprintf("OSD: %3d", osd_time));
    osd_print(row++, 1, tprintf("PPS: %3d", packetpersecond));
    osd_refresh();
}
