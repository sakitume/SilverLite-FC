...
static void update_osd()
{
    ...
    for (int i=0; i<10; i++)
    {
        osd_print(row++, 1, tprintf("FMC(%2d): %d", i, fmc_values[i]));
    }
    osd_refresh();
}

//------------------------------------------------------------------------------
static void flash_save(uint32_t offset)
{
	fmc_unlock();
	fmc_write( 0, 0 );
	fmc_erase();
    for (uint32_t addresscount = 0; addresscount < 10; addresscount++)
    {
        fmc_write(addresscount, (offset + addresscount));
    }
    fmc_lock();
}
...
static void flash_load()
{
    for (uint32_t addresscount = 0; addresscount < 10; addresscount++)
    {
		fmc_values[addresscount] = fmc_read(addresscount);
    }
}
